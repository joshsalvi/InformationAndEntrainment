...
for p = 1:numfreq
    ...
    MI_phase(p,i,j) = mutualinformation3(x11,y11);    
...
for k = 1:100       % 1%-100% of signal
    ...
    MI_displ_conv(k,p,i,j) = mutualinformation2(x11,y11);
    MI_phase_conv(k,p,i,j) = mutualinformation3(x11,y11);  
...
figure;
[hAx,hLine1,hLine2]=plotyy(amp_stim,VS_mean,amp_stim,MI_displ_mean);
hold(hAx(1),'on');errorbar(hAx(1),amp_stim,VS_mean,VS_sem);
hold(hAx(2),'on');errorbar(hAx(2),amp_stim,MI_displ_mean,MI_displ_sem);
